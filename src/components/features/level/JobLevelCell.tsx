...
export const JobLevelCell = memo(({ charIndex, jobId }: JobLevelCellProps) => {
  const level = useCharacterStore(
    (state) =>
      state.characters[charIndex].levels.find((l) => l.jobId === jobId)
        ?.level ?? 0,
  );
  ...
  return (
    <div className={styles.jobRow}>
      ...
      <JobLevelInput
        value={level}
        onChange={(newLevel) => updateLevel(charIndex, jobId, newLevel)}
        maxLevel={maxLevel}
